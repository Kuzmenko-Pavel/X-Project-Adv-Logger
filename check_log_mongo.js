...
if (!db.log.impressions.validate().valid || !db.log.impressions.stats().capped) {
    ...
    db.createCollection('log.impressions', {capped: true, max: 50000000, size: 50000000 * 500});
...
if (!db.log.impressions.block.validate().valid || !db.log.impressions.block.stats().capped) {
    ...
    db.createCollection('log.impressions.block', {capped: true, max: 50000000, size: 50000000 * 500});
}
else {
...
if (!db.log.retargeting.validate().valid || !db.log.retargeting.stats().capped) {
    ...
    db.createCollection('log.retargeting', {capped: true, max: 50000000, size: 50000000 * 500});
}
else {
